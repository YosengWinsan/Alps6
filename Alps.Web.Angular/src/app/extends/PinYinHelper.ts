...
static readonly phoneticTranscriptionObj = {
	"a": "\u554a\u963f\u9515",
...
// 汉字转拼音
static ConvertPinyin(ChineseCharacter) {
	let ChineseCharacterLength = ChineseCharacter.length;
	let phoneticTranscription = "";
	let reg = new RegExp('[a-zA-Z0-9\- ]');
	for (let i = 0; i < ChineseCharacterLength; i++) {
		let val = ChineseCharacter.substr(i, 1);
		let name = PinYinHelper.arraySearch(val,  PinYinHelper.phoneticTranscriptionObj);
		if (reg.test(val)) {
...
// 首字母大写
private static ucfirst(initial) {
	if (initial.length > 0) {
		let first = initial.substr(0, 1).toUpperCase();
		//let spare = initial.substr(1, initial.length);
		// 返回首字母大写的全拼
		//return first + spare;
		// 只返回首字母
		 return first;
